...
loop:
    BSF PORTD, 0
    BTFSS INTCON, 2 ;REVISAMOS QUE ((INTCON) and 07Fh), 2 SE ACTIVE
    GOTO loop
...
CONFIG_IO: ;CONFIGURACION DE PUERTOS
    ...
    BSF TRISA, 0 ;PIN ((TRISA) and 07Fh), 0 COMO INPUT (PUSH1)
    BSF TRISA, 1 ;PIN ((TRISA) and 07Fh), 1 COMO INPUT (PUSH2)
...
CONFIG_TMR0: ;CONFIGURACION DE TMR0
    ...
    MOVLW 11010000B ;SELECCIONAMOS SÓLO LOS BITS ((OPTION_REG) and 07Fh), 3,((OPTION_REG) and 07Fh), 2,((OPTION_REG) and 07Fh), 1,((OPTION_REG) and 07Fh), 0, TOSC
...
;----------------------SUB RUTINAS DE FUNCIONES ---------------------------------
 ...
 REINICIO_TMR0: ;TEMPORIZACION 2ms
    MOVLW 6 ;(256-N) N = 6
    MOVWF TMR0 ;LE COLOCAMOS 131, PARA LUEGO OBTENER 2 ms
    BCF INTCON, 2 ;LIMPIAMOS NUESTRA BANDERA DE ((INTCON) and 07Fh), 2, PARA QUE
    RETURN
