...
;
; Programa: CONTADOR DISPLAY
; Hardware: PUSH EN EL PUERTO B, LEDs en puerto C,DISPLAY 7 SEGEMENTOS EN EL PUERTO D
;
; Creado: 22 feb, 2021
...
; CONFIGURATION WORD 2
  ...
  DECR EQU 1 ;VALOR DEL PUSH DE DECREMENTO
   ...
   PSECT udata_bank0 ;common memory
    ...
    TABLA: DS 1 ;TABLA DE 1 BYTE PARA EL DISPLAY 1
...
;-------------------------VECTOR INTERRUPT -------------------------------------
 ...
 ISR:
    ...
    BTFSC ((INTCON) and 07Fh), 2 ;REVISAMOS LA BANDERA DE OVERFLOW DEL TMR0
    CALL CONT_DISPLAY ;LLAMAMOS A LA SUBRUTINA DE CONTEO DEL DYSPLAY
...
CONFIG_IO: ;CONFIGURACION DE PUERTOS
    ...
    CLRF TABLA ;LIMPIAMOS NUESTRAS VARIABLES
    CLRF CONT
    CLRF DISP
    RETURN
...
;-----------------------SUB RUTINAS DE INTERRUPCION ---------------------------
 INC_LED:
    ...
    MOVWF PORTC ;MOVEMOS EL VALOR DE W A PORTC
    BTFSC PORTA, 4 ;NECESITAMOS QUE SOLO SEA DE 4 BITS
    CLRF PORTA ;LIMPIAMOS PORTA PARA REINICIO DE CONTEO
    BCF ((INTCON) and 07Fh), 0 ;LIMPIAMOS LA BANDERA DE ((INTCON) and 07Fh), 0
    RETURN

 CONT_DISPLAY:
    CALL TMR0_RESTART ;LLAMAMOS A RESETEAR TMR0
    INCF CONT ;INCREMENTAMOS NUESTRA VARIABLE
    MOVF CONT, W ;MOVEMOS EL VAROR A W
    SUBLW 125 ;RESTA CON 125, QUEREMOS QUE SE REPITA 125 VECES
    BTFSS ((STATUS) and 07Fh), 2 ;REVISAMOS BANDERA DE ((STATUS) and 07Fh), 2
    RETURN
    CLRF CONT ;LIMPIAMOS LA VARIABLE DE CONTEO
    INCF DISP ;INCREMENTAMOS A LA VARIABLE DE DISP
    MOVF DISP, 0 ;MOVEMOS EL VALOR A W
    ANDLW 0X0F ;SOLO DEJA CONTAR HASTA 16 DIGITOS
    CALL TABLA_DISPLAY ;VAMOS A OBTENER EL VALOR A NUESTRA TABLA
    MOVWF PORTD ;MOVEMOS EL VALOR DE W A PORTD
    BCF ((INTCON) and 07Fh), 2 ;LIMPIAMOS NUESTRA BANDERA
    RETURN
  RETURN1:
    RETURN
