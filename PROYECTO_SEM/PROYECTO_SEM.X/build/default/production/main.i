...
;-------------------------VECTOR RESET ----------------------------------------
 ...
 ISR:
    BTFSC ((INTCON) and 07Fh), 0 ;REVISAMOS LA BANDERA DE CHANGE INTERRUPT
    CALL CAMBIAR_MODO ;LLAMAMOS A LA SUBRUTINA DE MODO


    BTFSC ((INTCON) and 07Fh), 2 ;REVISAMOS LA BANDERA DE TOIF
    CALL TIEMPO1 ;LLAMAMOS A LA SUBRUTINA DEL TIEMPO
    CALL DISP_VIA1 ;LLAMAMOS A LA SUBRUTINA DEL DISPLAY
    BTFSC ((PIR1) and 07Fh), 1 ;REVISAMOS LA BANDERA ((PIR1) and 07Fh), 1
 ...
 POP:
    ...
    RETFIE ;VUELVE ACTIVAR EL ((INTCON) and 07Fh), 7
...
CONFIG_TMR0: ;CONFIGURACION DE TMR0
    ...
    MOVLW 01010000B ;SELECCIONAMOS SÓLO LOS BITS ((OPTION_REG) and 07Fh), 3(0),((OPTION_REG) and 07Fh), 2,((OPTION_REG) and 07Fh), 1,((OPTION_REG) and 07Fh), 0, TOSC(0), a RBPU(0) HABILITA PULL UP
...
CONFIG_INT_ENABLE:
    BSF ((INTCON) and 07Fh), 7 ;HABILITAMOS LAS INTERRUPCIONES GLOBALES
    BSF ((INTCON) and 07Fh), 6 ;HABILITAMOS LAS INTERRUPCIONES PERIFERICAS
    BSF ((INTCON) and 07Fh), 3 ;HABILITAMOS LAS INTERRUPCIONES INTERRUPT-ON-CHANGE
    BSF ((INTCON) and 07Fh), 5 ;HABILITAMOS LA INTERRUPCION DE OVERFLOW DE TMR0
    BCF ((INTCON) and 07Fh), 2 ;LIMPIAMOS LA BANDERA DEL TMR0
    BCF ((INTCON) and 07Fh), 0 ;LIMPIAMOS LA BANDERA DE INTERRUPT-ON-CHANGE
    BANKSEL OPTION_REG
    BSF PIE1, 1 ;HABILITAMOS LA INTERRUPCION DE OVERFLOW DE TMR2
    BANKSEL TMR0
    BCF ((PIR1) and 07Fh), 1 ;LIMPIAMOS LA BANDERA DEL TMR2
    RETURN
...
CONFIG_IOCRB:
 ...
 ;-----------------------SUB RUTINAS DE INTERRUPCION --------------------------
  TMR0_RESTART:
    ...
    BCF INTCON, 2 ;LIMPIAMOS NUESTRA BANDERA DE ((INTCON) and 07Fh), 2
    RETURN
  ...
  TIEMPO1:
    ...
    BTFSS ((STATUS) and 07Fh), 2 ;REVISAMOS BANDERA DE ((STATUS) and 07Fh), 2
    RETURN ;REGRESAMOS
    ...
    LED_BLINK:
    BCF ((PIR1) and 07Fh), 1 ;LIMPIAMOS LA INTERRUPCION
 ...
 COLOR_RED3:
  ...
  CAMBIAR_MODO:
    BCF STATUS, 0 ;LIMPIAMOS LA BANDERA DE ((STATUS) and 07Fh), 0 PARA QUE NO AFECTE
    ...
    BSF FLAG_ID, 1
    BCF ((INTCON) and 07Fh), 0 ;LIMPIAMOS LA BANDERA DE LOS PUSH
    MOVLW 00100000B
    XORWF SMODO, W ;QUE LA VARIABLE NO PASE DE 6, SOLO EXISTEN 5 MODOS
    BTFSS ((STATUS) and 07Fh), 2 ;REVISAMOS LA VARIABLE DE ((STATUS) and 07Fh), 2
    RETURN
    MOVLW 00000001B
    MOVWF SMODO
    RETURN
 ...
 MODE_1:
  ...
  DEC1:
    ...
    BTFSS ((STATUS) and 07Fh), 2
    RETURN
    MOVLW 0X14 ;COLOCAR EL VALOR DE 10 A LA VARIABLE
    MOVWF C_VIA1
    RETURN
  MODE_2:
    BCF PORTE, 0
  ...
  MODE_4:
    ...
    RETURN
  ACEPTAR:
    BTFSS FLAG_ID, 0 ;BANDERA DE ANTIREBOTE DE DEC
    RETURN
    BCF FLAG_ID, 0
    MOVF C_VIA1, W
    MOVWF TVIA1, F
  ...
  CANCEL:
    ...
    RETURN
    BCF FLAG_ID, 1 ;LIMPIAMOS LA BANDERA
    MOVF TVIA1, W ;VOLVEMOS A CARGAR LAS VARIABLES ORIGINALES
    MOVWF C_VIA1
    MOVF TVIA2, W
    MOVWF C_VIA2
    MOVF TVIA3, W
 ...
 SEMAFORO1:
    ...
    BTFSC ((STATUS) and 07Fh), 2 ;REVISAMOS BANDERA DE ((STATUS) and 07Fh), 2
    BSF SEM_COLOR, 0
    MOVLW 0X04
    XORWF T_VIA1, W ;CUANDO LLEGUE A 4 TIENE QUE CAMBIAR AMARILLO
    BTFSC ((STATUS) and 07Fh), 2 ;REVISAMOS BANDERA DE ((STATUS) and 07Fh), 2
 ...
 DECENAS_1:
    ...
    BTFSC STATUS, 0 ;VERIFIACMOS LA BANDERA DE ((STATUS) and 07Fh), 0
